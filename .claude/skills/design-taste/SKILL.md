...
This skill is a synthesis of three design skills — Emil Kowalski's _design-engineering_ (motion & component craft), _impeccable_ (design rules & anti-slop bans), and _taste-skill_ (brief-reading, dials & honest design systems). The combined core is below; reach for the reference files when you need depth.
...
> _"Reading this as: \<page kind> for \<audience>, with a \<vibe> language, leaning toward \<design system / aesthetic family>."_
...
- UI animations stay under 300ms. Use **ease-out** for enter/exit (responsive); never `ease-in` on UI. Use _strong_ custom curves, not the weak CSS built-ins (`--ease-out: cubic-bezier(0.23, 1, 0.32, 1)`).
