...
import { authFailuresTotal } from "@saasweave/observability";
...
export const authMiddleware = createMiddleware().server(async ({ next }) => {
  ...
  if (!user) {
    authFailuresTotal.inc({ reason: "missing_session" });
...
export const freshAuthMiddleware = createMiddleware().server(async ({ next }) => {
  ...
  if (!user) {
    authFailuresTotal.inc({ reason: "missing_or_stale_session" });
    setResponseStatus(401);
    throw new Error("Unauthorized");
  }
