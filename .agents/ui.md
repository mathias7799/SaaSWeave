...
- Prefer existing primitives from `@saasweave/ui` before creating new app-local ones.
...
- Prefer existing components from `@saasweave/ui/components/*` before creating new app-local primitives.
...
- Use `@saasweave/ui/lib/utils` `cn(...)` for class composition.

## App Composition

- Build page UI in `pages/`, composite sections in `widgets/` and `features/`, and app-level primitives in `shared/ui`.
- Prefer composing `@saasweave/ui` primitives instead of duplicating styling across many leaf components.
...
- `packages/ui` exposes explicit component/hook/style subpaths; do not add a wildcard export or import package internals outside `package.json#exports`.
...
- Keep reusable app composites that depend on product copy, routing, permissions, or domain records in `apps/web/src/shared/ui`; they are not generic `packages/ui` primitives merely because several pages use them.
