...
Use `evlog` through `@saasweave/logger` only. Do not install or import `evlog` directly from `apps/*` or feature packages; the logger package is the facade and adapter boundary.
...
```ts
import { parseError } from "@saasweave/logger/server";
...
```ts
import { createError } from "@saasweave/logger/server";
...
Use Better Auth identification through evlog's Better Auth integration when request identity is needed. Keep that integration behind `@saasweave/logger`; do not import `evlog/better-auth` directly from app or feature code unless the logger package is being updated.
...
Use evlog's centralized `redact` configuration in `@saasweave/logger/server`; do not create one-off redaction helpers at individual callsites. If a new sensitive field family appears, add it to the shared logger redaction config instead of masking it manually in feature code.
...
Prometheus metrics are separate from durable logs and live in `@saasweave/observability`. Use bounded, stable labels such as method, normalized route, status class, queue, and job name; never use tenant IDs, request IDs, URLs, or arbitrary error text as metric labels. Distributed tracing is not configured, so do not claim trace coverage when only request-ID log correlation exists.
...
Browser log ingestion uses **anonymous same-origin ingestion with optional session enrichment**. See [packages/logger/INGEST-AUTH.md](../packages/logger/INGEST-AUTH.md) for limits, server-owned fields, rate limiting, and Redis failure policy.

## Bootstrap Console

Bootstrap/config packages such as `@saasweave/env` may use small `console.debug` statements because they are loaded before logger initialization and by Vite config. Do not route env bootstrap through `@saasweave/logger`.
